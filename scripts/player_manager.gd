...
#@onready var get_viewport().get_camera_3d() : Camera3D = get_viewport().get_camera_3d()

var deployedAgents : Array

func _ready():
	if(Global.force_load_level):
		Global.load_level(Global.force_load_level)
		Global.force_load_level = null
		await Global.on_level_start
